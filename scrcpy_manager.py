...
import os, json, shutil, zipfile, urllib.request, threading
...
def get_scrcpy_path():
    """Find scrcpy.exe: app dir > PATH > None."""
    local = os.path.join(SCRCPY_DIR, "scrcpy.exe")
...
def download_and_install(url, progress_cb=None, done_cb=None):
    """Download zip from url, extract to SCRCPY_DIR."""
    def run():
        try:
            os.makedirs(APP_DIR, exist_ok=True)
