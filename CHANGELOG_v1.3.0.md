...
**Fecha:** Agosto 2026  
**Versión:** 1.3.0 "IDE Edition & Smart Hardware Intelligence"
...
- **Barra de Navegación Superior Unificada (Topbar)**:
  - Selector de dispositivos en tiempo real.
  - Botones de diagnóstico de hardware directos (Encoders, Cámaras, Pantallas, Modo PC).
  - Botón de actualización de scrcpy y botones de control de ejecución (Iniciar Scrcpy / Detener).
- **Barra Lateral de Actividades (Sidebar)**:
  - Explorador vertical de modos y presets con badges descriptivos y categorización visual.
...
- **Espacio de Trabajo Central Adaptativo (Center Workspace)**:
  - **Ficha Técnica / Inspector de Preset**: Muestra métricas clave en 4 tarjetas de estadísticas (Códec, Resolución, FPS, Bitrate), resumen de características y panel de interruptores rápidos de sesión.
  - **Editor de Propiedades Avanzado**: Pestañas de configuración personalizada organizadas en dos columnas balanceadas.
- **Dock de Terminal Integrado (Terminal Console Dock)**:
  - Consola estilo terminal con tipografía monoespaciada para logs y comandos generados en tiempo real.
  - Botones de utilidad: Copiar CLI y Limpiar consola.
...
- **Análisis de Hardware Automático en Segundo Plano**:
  - Al conectar o seleccionar un dispositivo, ScrcpyGUI analiza de forma asíncrona sus capacidades reales mediante ADB y scrcpy:
    ...
    - **Versión de Android y SoC**: Identifica la versión de Android y nivel de API para validar compatibilidad con UHID, Audio forwarding y Modo Escritorio.
  - Sistema de caché por número de serie para respuestas instantáneas.

---

### 3. Preset "App en Pantalla Virtual" y Selector Integrado de Apps
- **Preset Dedicado**: "App en Pantalla Virtual" para abrir aplicaciones en un monitor virtual secundario en resolución 1080p con DPI de tablet y control UHID.
- **Selector de Aplicaciones Inline**:
  - Permite filtrar y buscar en tiempo real por nombre de aplicación o paquete directamente en la interfaz principal, sin necesidad de ventanas emergentes o diálogos flotantes.
...
- **Estabilidad de Pestañas**: Fijado el alineamiento de pestañas en `CTkTabview` con layouts balanceados en 2 columnas uniformes para evitar desplazamientos horizontales.
...
- **Manejo de Errores de Descarga**: Corregido el manejador de callbacks de descarga para evitar excepciones en hilos secundarios.
- **Suite de Pruebas Automatizadas**: 40 pruebas unitarias completas pasando al 100% (`tests/test_command_builder.py`, `tests/test_config_manager.py`, `tests/test_scrcpy_manager.py`, `tests/test_preset_filtering.py`, `tests/test_paths.py`).

---

*Desarrollado por zTheNea — ScrcpyGUI v1.3.0*
